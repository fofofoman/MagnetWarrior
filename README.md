...
| | 이전 | 현재 |
| --- | --- | --- |
| 스크래퍼 (60→115 HP) | 80스터드 밖에서도 처박아 죽임 | **46스터드 안까지 붙어야** 죽음 |
| 스크래퍼 한 마리에 붙잡히면 | 9.9초 버팀 | **4.3초** |
| 하울러 한 마리에 붙잡히면 | 5.5초 | **3.0초** |
| 환경 위험 | 배치 0개 (시스템만 존재) | **8스테이지 22곳** |
| 수집기 완벽 클리어 | 29초 (창 4.0초) | **43초** (창 3.0초) |
| 대자석로 완벽 클리어 | 55초 (창 3.5초) | **72초** (창 2.8초) |
| 브론즈 | 골드 +90% — 완주하면 받음 | **골드 +60%** |

**한 방 킬은 일부러 남겼다.** 붙잡아서 벽에 처박는 손맛이 이 게임의 핵심이고, 체력을 올려
모든 적을 스펀지로 만들면 난이도가 아니라 지루함이 된다. 대신 *멀리서 대충 쏜 것*이 안 죽게
했다 — Push는 거리에 따라 감쇠하므로, 체력을 올린다는 건 **죽일 수 있는 거리를 좁힌다**는
뜻이다. 압박은 접촉 데미지가 담당한다. 아레나는 클리어가 느려진 게 아니라 서 있기 위험해졌다.

**환경 위험은 전부 통과 가능하다.** 깊이 6스터드 이하는 뛰어넘을 수 있고, 그보다 넓은 것은
최소 10스터드의 우회로가 옆에 있다. 체크포인트·소켓·보급 선반 위에는 하나도 놓지 않았다 —
플레이어가 반드시 서 있어야 하는 자리를 아프게 만드는 건 선택지가 아니라 세금이다.
`scratchpad/invariants.py`가 22곳 전부를 검사한다.

**1·2·7·11면은 일부러 비워뒀다.** 1·2는 튜토리얼, 7은 순수 타이밍(낙사가 이미 벌칙),
11은 수직 등반이라 발판 하나하나가 체크포인트다.

### 알아둘 것

- **하울러는 벽에 처박아 죽일 수 없다.** 질량 108이라 슬램으로는 17 데미지(220 HP)밖에 안 나온다.
