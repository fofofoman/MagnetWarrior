...
### 스테이지 절전 (`StageActivity`)

12개 스테이지가 하나의 place에 전부 지어져 있으므로, 그대로 두면 서버 예산의 대부분이 아무도
보고 있지 않은 벽을 움직이고 2만 스터드 떨어진 플레이어에게 드론을 날리는 데 쓰인다.

`StageActivity`는 "이 스테이지 근처에 사람이 있는가" 하나만 답하고, 적 AI·움직이는 벽·보스
패턴이 일하기 전에 이걸 확인한다. Players 외에 아무것도 require하지 않는 잎 모듈이라 (이미
서로를 require하고 있는) 어느 시스템에서 불러도 순환이 생기지 않는다.

깨우는 반경(900 스터드)이 넉넉한 것은 의도적이다. 보이기 시작할 때 벽이 움직이기 시작하면
눈에 띄게 튀므로, 시야에 들어오기 한참 전에 깨워 둔다. 벽의 위상은 절대 시각에서 계산되므로
자다 깨어나도 원래 있어야 할 위치로 이어진다 — 누적 오차가 없다.
...
코어 뽑기 판정은 **서버가 실제로 가한 임펄스**로 한다(`MagneticRegistry.Impulsed`). 뻔한 두
구현은 둘 다 실패한다. 코어의 속도를 표본화하면 스파이크를 놓친다 — Pull의 속도 변화는 몇
프레임이고 보스 루프는 15 Hz다. 코어가 받침대에서 얼마나 벗어났는지를 재는 방법은 동작하지만,
그 거리는 AlignPosition 스프링이 하중 아래에서 어떻게 거동하는지에 전적으로 달려 있어서
코드만으로는 확정할 수 없고 실제 엔진에서 맞춰봐야 하는 값이 된다. 임펄스는 서버가 스스로
계산한 값이므로 정확하고, 프레임률과 무관하며, 제약 조건을 어떻게 튜닝하든 영향받지 않는다.
